...
export default function FollowUpStoreMediaPage({ params }: { params: Promise<{ publicId: string }> }) {
  ...
  const loadStore = async () => {
    try {
      setIsLoading(true);
      const res = await followUpStoresService.getStore(resolvedParams.publicId);
      setStore(res);
    } catch (err) {
  ...
  const handleUpload = async (type: 'logo' | 'cover' | 'gallery', file: File) => {
    try {
      if (type === 'logo') {
        await followUpStoresService.uploadLogo(resolvedParams.publicId, file);
      } else if (type === 'cover') {
        await followUpStoresService.uploadCover(resolvedParams.publicId, file);
      } else if (type === 'gallery') {
        await followUpStoresService.uploadGalleryImage(resolvedParams.publicId, file);
      }
      await loadStore();
    } catch (err: any) {
  ...
  const handleDelete = async (type: 'logo' | 'cover' | 'gallery', mediaId?: string) => {
    ...
    try {
      if (type === 'logo') {
        await followUpStoresService.deleteLogo(resolvedParams.publicId);
      } else if (type === 'cover') {
        await followUpStoresService.deleteCover(resolvedParams.publicId);
      } else if (type === 'gallery' && mediaId) {
        await followUpStoresService.deleteGalleryImage(resolvedParams.publicId, mediaId);
      }
      await loadStore();
    } catch (err) {
      alert("فشل حذف الملف.");
      ...
      <div className="bg-white dark:bg-[#171717] rounded-2xl p-6 border border-[#EAF3EC] dark:border-[#1F2522]">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500">أضف صوراً إضافية للمحل لعرضها للزبائن</p>
          </div>
