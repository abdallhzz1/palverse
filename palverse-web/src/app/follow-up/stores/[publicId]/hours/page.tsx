...
export default function FollowUpStoreWorkingHoursPage({ params }: { params: Promise<{ publicId: string }> }) {
  ...
  useEffect(() => {
    followUpStoresService.getWorkingHours(resolvedParams.publicId)
      .then(res => {
        const initHours = { ...res };
        DAYS.forEach(day => {
          if (!initHours[day.id]) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await followUpStoresService.updateWorkingHours(resolvedParams.publicId, hours);
      setSuccess(true);
    } catch (err: any) {
