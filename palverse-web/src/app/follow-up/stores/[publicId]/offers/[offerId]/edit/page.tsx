...
export default function FollowUpEditOfferPage({ params }: { params: Promise<{ publicId: string; offerId: string }> }) {
  ...
  useEffect(() => {
    followUpStoresService.getOffer(resolvedParams.publicId, resolvedParams.offerId)
      .then(offer => {
        setFormData({
      ...
      .catch(() => setError("فشل تحميل العرض"))
      .finally(() => setIsLoading(false));
  }, [resolvedParams.publicId, resolvedParams.offerId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await followUpStoresService.updateOffer(resolvedParams.publicId, resolvedParams.offerId, payload);
      router.push(`/follow-up/stores/${resolvedParams.publicId}/offers`);
    } catch (err: any) {
      if (err.data?.errors) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        <Link href={`/follow-up/stores/${resolvedParams.publicId}/offers`} className="p-2 hover:bg-gray-200 dark:hover:bg-gray-800 rounded-full transition-colors">
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col gap-4">
              <div className="h-48 w-full relative rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden flex items-center justify-center bg-gray-50 dark:bg-[#252525]">
                {imagePreview ? (
                  <Image src={imagePreview} alt="Preview" fill unoptimized={true} className="object-cover" />
                ) : (
