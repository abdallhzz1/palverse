...
import type { FollowUpStoreOffer } from "@/types/followUpStore";

export default function FollowUpStoreOffersPage({ params }: { params: Promise<{ publicId: string }> }) {
  const resolvedParams = use(params);
  const [offers, setOffers] = useState<FollowUpStoreOffer[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  const loadOffers = async () => {
    try {
      const res = await followUpStoresService.getOffers(resolvedParams.publicId);
      setOffers(res.data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleDelete = async (offerId: string) => {
    ...
    try {
      await followUpStoresService.deleteOffer(resolvedParams.publicId, offerId);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Link
          href={`/follow-up/stores/${resolvedParams.publicId}/offers/new`}
      ...
      {offers.length === 0 ? (
        <div className="bg-white dark:bg-[#171717] rounded-2xl border border-[#EAF3EC] dark:border-[#1F2522] p-12 text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-6 max-w-md mx-auto">
            أضف عروضاً وتخفيضات لجذب المزيد من الزبائن لهذا المحل.
          </p>
          <Link
            href={`/follow-up/stores/${resolvedParams.publicId}/offers/new`}
                ...
                <div className="p-5 flex-1 flex flex-col">
                  ...
                  <div className="mt-4 flex flex-col gap-1 text-xs text-gray-500">
                    <div className="flex items-center gap-1">
                      ...
                      <span>يبدأ: {offer.starts_at ? new Date(offer.starts_at).toLocaleDateString("ar-SA") : "غير محدد"}</span>
                    </div>
                    <div className="flex items-center gap-1 text-red-400">
                      <Calendar className="w-3 h-3" />
                      <span>ينتهي: {offer.ends_at ? new Date(offer.ends_at).toLocaleDateString("ar-SA") : "غير محدد"}</span>
                    </div>
                  </div>

                  <div className="mt-6 pt-4 border-t border-[#EAF3EC] dark:border-[#1F2522] flex justify-between items-center mt-auto">
                    <Link
                      href={`/follow-up/stores/${resolvedParams.publicId}/offers/${offer.public_id}/edit`}
