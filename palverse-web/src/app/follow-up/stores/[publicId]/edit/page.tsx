...
export default function FollowUpStoreEditPage({ params }: { params: Promise<{ publicId: string }> }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const storeData = await followUpStoresService.getStore(resolvedParams.publicId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await followUpStoresService.updateStore(resolvedParams.publicId, payload);
      setSuccess(true);
        ...
        {isRejected && (
          <div className="bg-yellow-50 text-yellow-800 p-4 rounded-lg text-sm font-bold border border-yellow-200 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="font-normal mt-1">تعديل البيانات وحفظها سيؤدي إلى إعادة إرسال المحل للمراجعة تلقائياً.</p>
            </div>
          </div>
        )}
