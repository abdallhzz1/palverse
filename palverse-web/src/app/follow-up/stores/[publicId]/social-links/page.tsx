...
import { Link as LinkIcon, Plus, Trash2, Edit2, X } from "lucide-react";
import { followUpStoresService } from "@/services/followUpStores.service";
import type { FollowUpStoreSocialLink } from "@/types/followUpStore";
...
export default function FollowUpStoreSocialLinksPage({ params }: { params: Promise<{ publicId: string }> }) {
  const resolvedParams = use(params);
  const [links, setLinks] = useState<FollowUpStoreSocialLink[]>([]);
  ...
  const loadLinks = async () => {
    try {
      const res = await followUpStoresService.getSocialLinks(resolvedParams.publicId);
      setLinks(res);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const openModal = (link?: FollowUpStoreSocialLink) => {
    if (link) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await followUpStoresService.updateSocialLink(resolvedParams.publicId, editingId, formData);
      } else {
        await followUpStoresService.addSocialLink(resolvedParams.publicId, formData);
      }
      await loadLinks();
      closeModal();
    } catch (err: any) {
  ...
  const handleDelete = async (linkId: string) => {
    ...
    try {
      await followUpStoresService.deleteSocialLink(resolvedParams.publicId, linkId);
