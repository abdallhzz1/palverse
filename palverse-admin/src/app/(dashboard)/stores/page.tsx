  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">إدارة المحلات</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">عرض وإدارة المحلات واعتمادها أو رفضها</p>
        </div>
      </div>
        ...
        <div className="p-4 border-b border-slate-200 dark:border-slate-800">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <select
                className="h-10 px-3 py-2 rounded-md border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"
                value={params.status || ""}
          ...
          <table className="w-full text-sm text-right">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {isLoading ? (
                ...
                data?.data.map((store) => (
                  <tr key={store.public_id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded border border-slate-200 dark:border-slate-700 overflow-hidden bg-slate-100 dark:bg-slate-800 flex items-center justify-center shrink-0">
                          {store.logo ? (
                    ...
                    <td className="px-6 py-4">
                      {store.owner ? (
                        <>
                          ...
                          <div className="text-xs text-slate-500 mt-1">{store.owner.email}</div>
                        </>
                      ) : (
                    ...
                    <td className="px-6 py-4">
                      <div className="font-medium">{store.city ? store.city.name_ar : <span className="text-slate-400 text-xs">غير محدد</span>}</div>
                      <div className="text-xs text-slate-500 mt-1">{store.category ? store.category.name_ar : <span className="text-slate-400 text-xs">غير محدد</span>}</div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <Link 
                        ...
                        className="inline-flex items-center justify-center h-8 px-3 rounded-md text-xs font-medium bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
                      >
                        عرض التفاصيل
                      </Link>
                    </td>
