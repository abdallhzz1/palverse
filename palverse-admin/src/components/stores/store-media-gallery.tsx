  ...
  return (
    <div className="space-y-6">
      ...
      {gallery && gallery.length > 0 && (
        <div>
          <h4 className="text-sm font-bold text-foreground dark:text-white mb-4">معرض الصور</h4>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {gallery.map((media) => (
              <div key={media.public_id} className="aspect-square rounded-lg border border-border dark:border-slate-700 overflow-hidden bg-muted dark:bg-slate-900 relative">
                <img 
                  src={media.url} 
                  alt="صورة من المعرض" 
                  className="w-full h-full object-cover"
