...
export function OfferPeriod({ offer, className }: { offer: AdminOffer; className?: string }) {
  if (!offer.starts_at && !offer.ends_at) {
    return <span className={cn("text-sm text-muted-foreground", className)}>مستمر دون تاريخ انتهاء</span>;
  }
  ...
  return (
    <div className={cn("text-sm text-muted-foreground flex flex-col gap-0.5", className)}>
      {startStr && endStr ? (
